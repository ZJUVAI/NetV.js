...
class RadialTree {
    ...
    start() {
        var _a, _b;
        if ((_a = this._data.nodes) === null || _a === void 0 ? void 0 : _a.length) {
...
class Chaos {
    ...
    start() {
        if (this._stopped)
            return; // should not start when the layout was stopped
        this._timeInterval = setInterval(this._timerHandler, this._param.timeout);
    }
    restart() {
        if (!this._stopped)
            this.stop(); // if this layout is not stopped, stop it
        this._stopped = false;
    ...
    stop() {
        var _a;
        if (this._stopped)
            return; // the layout has already been stopped
        this.pause();
    ...
    resume() {
        if (this._timeInterval)
            return; // should not resume when the layout was not paused
        this.start();
    }
    pause() {
...
class ForceAtlas2 {
    ...
    stop() {
        var _a, _b;
        if (this._stopped)
            return this; // the layout has already been stopped
        this._running = false;
        this._stopped = true;
        this._matrices = null;
        if (this._param.useWorker) {
    ...
    resume() {
        if (this._running)
            return this; // the layout is running
        this.start();
    }
    pause() {
        this._running = false;

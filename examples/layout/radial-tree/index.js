...
const layout = radialLayoutMultiple(data, 'Valjean', { width, height })
const id2layout = layout.nodes.reduce((r, n) => {
    r[n.id] = n
    return r
}, {})
const colorMap = [
...
data.nodes.forEach((node) => {
    ...
    node.x = id2layout[node.id].x
    node.y = id2layout[node.id].y
})
